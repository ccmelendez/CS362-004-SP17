...
int checkPlayAdventurer(struct gameState *state,int currentPlayer,int *drawntreasure,int *temphand,int z) {
  int r;

  
  //r = playAdventurer(drawntreasure,state,currentPlayer,cardDrawn,temphand,z);
  r=adventurerEffect(state,currentPlayer,drawntreasure,temphand,z);
...
int main () {
  ...
  //for (n = 0; n < 89; n++) {	  
	...
	checkPlayAdventurer(&G,randomPlayer,&drawnTreasure,testTemphand,z);
